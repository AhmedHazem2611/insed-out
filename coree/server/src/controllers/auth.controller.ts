...
const jwtSecret: Secret = (process.env.JWT_SECRET || "change-me") as Secret;
...
export async function registerHandler(req: Request, res: Response) {
	...
	try {
		...
		const inserted = await runQuery<{ id: string; email: string; name: string }>(
...
export async function loginHandler(req: Request, res: Response) {
	...
	try {
		const result = await runQuery<{ id: string; email: string; name: string; password_hash: string }>(
...
export async function meHandler(req: AuthenticatedRequest, res: Response) {
	...
	try {
		const result = await runQuery<{ id: string; email: string; name: string }>(
...
export async function googleLoginHandler(req: Request, res: Response) {
	...
	try {
		...
		// Upsert user
		const found = await runQuery<{ id: string; email: string; name: string }>(
